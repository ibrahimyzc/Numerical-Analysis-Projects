%this code solves the advection-diffusion boundary value problem with the upwind finite difference method
...
%application of the upwind finite difference method
uapp=zeros(N,1);
uapp(1)=0;
uapp(N)=1;
A=zeros((N-2),(N-2));
for i=1:N-2
    A(i,i)=(2*eps)/(hstep^2)+(a/hstep);
end
for i=1:N-3
    A(i,i+1)=-(eps/(hstep^2));
end
for i=2:N-3
    A(i+1,i)=-(eps/(hstep^2))-(a/hstep);
end
c=zeros(N-2,1);
c(1)=0*(-(eps/(hstep^2))-(a/hstep));
c(N-2)=-(-(eps/(hstep^2)));
uapp2=A\c;
for i=1:N-2
    uapp(i+1)=uapp2(i);
